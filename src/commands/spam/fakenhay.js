...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({ ...embedMsg("🎭 FAKE NHẢY", `Target: **${impersonateObj.displayName}**\nDelay: **${delay}s**`, 0x9b59b6), ephemeral: true });
    ...
    const lines = readFileLines(getFilePath("nhay.txt"));
    if (!lines) {
      await interaction.channel.send({ embeds: [{ title: "❌ LỖI", description: "Thiếu file nhay.txt", color: 0xe74c3c }] });
    ...
    (async () => {
      ...
      try {
        ...
        await interaction.channel.send({ embeds: [{ title: "✅ FAKE NHẢY XONG", description: `Đã gửi **${sentCount}** dòng dưới tên **${impersonateObj.displayName}**`, color: 0x2ecc71 }] });
      } catch (err) {
        if (err.name !== "AbortError") logger.error(`Fake nhay: ${err.message}`);
      } finally {
